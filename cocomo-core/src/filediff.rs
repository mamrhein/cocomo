...
impl FileDiff {
    ...
    pub async fn new(
        ...
        let left_content = fs::read_to_string(left_file.path())?;
        let right_content = fs::read_to_string(right_file.path())?;
...
mod tests {
    ...
    async fn test_file_diff() {
        ...
        let diff = FileDiff::new(_l_item, _r_item).await.unwrap();
